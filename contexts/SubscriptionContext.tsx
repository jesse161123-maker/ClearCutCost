...
export function SubscriptionProvider({ children }: SubscriptionProviderProps) {
  ...
  const fetchOfferings = async () => {
    ...
    try {
      ...
      const defaultOffering =
        fetchedOfferings.all?.default ?? fetchedOfferings.current ?? null;

      if (defaultOffering) {
        setCurrentOffering(defaultOffering);
        ...
        setPackages([]);
        console.warn(
          "[RevenueCat] No default/current offering found. Available offerings:",
          Object.keys(fetchedOfferings.all ?? {}).join(", ") || "none"
        );
      }
    } catch (error) {
  ...
  const checkSubscription = async (): Promise<boolean> => {
    ...
    try {
      ...
      return hasEntitlement || (__DEV__ && isSubscribed);
    } catch (error) {
